...
SortPointer implicitConstructorSort(ContractDefinition const& _contract, SymbolicState& _state)
{
	return make_shared<FunctionSort>(
		vector<SortPointer>{_state.errorFlagSort(), _state.thisAddressSort(), _state.cryptoSort(), _state.txSort(), _state.stateSort()} + stateSorts(_contract),
		SortProvider::boolSort
	);
}
